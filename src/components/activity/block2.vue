<template>
    <div class="block_wrap_opc withTitle flex-col vbHidden" :class="{'animated fadeInLeft faster delay3':animate == 1,'animated zoomOut faster':animate == 2}">
        <div class="channelBar green">
            <div class="_title"><i class="ion">&#xe657;</i><strong>主题党日活动次数统计</strong></div>
        </div>
        <div class="flex-1 chartBox" style="margin:10px;">
            <pie :data="data" radius="40%,50%" :label="true" />
            <div class="comTotal" v-if="data.all_count">{{data.all_count}}次</div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        getData: function(){
            home.activitynum(this.queryParam).then(res => {
