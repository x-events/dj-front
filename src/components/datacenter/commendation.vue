  ...
  <!-- <div v-loading="loading">
    <div class="dialog-table scroll">
      ...
      <el-table :data="tableData"  stripe >
        <el-table-column prop="表彰名称" label="表彰名称">
        </el-table-column>
        <el-table-column prop="表彰批准日期" label="表彰批准日期" >
        </el-table-column>
        <el-table-column prop="授予荣誉称号级别" label="授予荣誉称号级别" >
        </el-table-column>
    ...
    <div class="data-edit right-form flex-col h100" v-if="formData">
        ...
        <div class="member-add-form-btn">
            <el-button type="default" icon="el-icon-circle-plus-outline" @click="addItem('form')">添加表彰信息</el-button>
        </div>
            ...
            <el-form :ref="'form'" :model="formData" label-width="190px" label-suffix=":">
                <div class="fors-item" v-for="(item, index) in formData.g" :key="index" :class="{'editStatus':item.isEdit}">
                    ...
                    <el-row class="form-row">
                        <el-col :span="12">
                            <el-form-item class="col-name" label="表彰名称" :prop="'g.'+index+'.commendname'" :rules="permissGroupRules.commendname">
                                <el-input v-model="item.commendname" readonly placeholder="请选择表彰名称" 
                                @focus="treepickerDialog=true;treeType='commend_name';treeTitle='表彰名称'" v-if="item.isEdit"></el-input>  
                                <span class="sblock" :title="item.commendname" v-else>{{item.commendname || ""}}</span>
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item class="col-name" label="表彰名称补充">
                                <el-input v-model="item.name_add" placeholder="请输入表彰名称补充" maxlength="50" v-if="item.isEdit"></el-input>
                                <span class="sblock" :title="item.name_add" v-else>{{item.name_add || ""}}</span>
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item class="col-name" label="表彰批准日期">
                        ...
                        <el-col :span="12">
                            <el-form-item class="col-name" label="授予荣誉称号级别">
                                <selectOpt v-model="item.honorary_title_level" placeholder="请选择级别" :option="honoraryData" @input="item.honorary_title_level=$event" v-if="item.isEdit" /> 
                                <span class="sblock" :title="item.honorary_title_level_name" v-else>{{item.honorary_title_level_name || ""}}</span>
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item class="col-name" label="表彰批准机构名">
                        ...
                        <el-col :span="12">
                            <el-form-item class="col-name" label="表彰批准机构级别">
                        ...
                        <el-col :span="24">
                            <el-form-item class="col-name" label="表彰原因">
                                <el-input v-model="item.commend_reason_name" readonly placeholder="请选择表彰原因" 
                                @focus="treepickerDialog=true;treeType='commend_reason';treeTitle='表彰原因'" v-if="item.isEdit"></el-input> 
                                <span class="sblock" :title="item.commend_reason_name" v-else>{{item.commend_reason_name || ""}}</span>
                            </el-form-item>
                        </el-col>
                        <el-col :span="24">
                            <el-form-item class="col-name" label="表彰说明">
                                <el-input v-model="item.remark" placeholder="请输入表彰说明" maxlength="200" type="textarea" :rows="3" v-if="item.isEdit"></el-input>
                                <span class="sblock" :title="item.remark" v-else>{{item.remark || ""}}</span>
                            </el-form-item>
                        </el-col>
                        <el-col :span="24">
                            <el-form-item class="col-name" label="表彰撤销日期">
                        ...
                        <el-col :span="24">
                            <el-form-item class="col-name" label="表彰撤销原因">
                                <selectOpt v-model="item.revoke_reason" placeholder="请选择表彰撤销原因" :option="revokeData" @input="item.revoke_reason=$event" v-if="item.isEdit" /> 
                                <span class="sblock" :title="item.revoke_reason_name" v-else>{{item.revoke_reason_name || ""}}</span>
                            </el-form-item>
                        </el-col>
                    </el-row>
                </div>
...
export default {
  ...
  props: {
    ...
    data() {
        return {
            ...
            permissGroupRules: {    // form (循环)rule
                commendname: [{required: true, message: '请选择表彰名称!', trigger: 'blur'}],
    ...
    methods: {
        /* getData() {
            this.loading = true
            datacenter.commend({uid: this.personId}).then( res => {
        ...
        getDicData() {
            // 授予荣誉称号级别
            common.getDictData_tree({id: 'honorary_title_level'}).then( res => {
                this.honoraryData = res.data.data
            })
            // 表彰批准机构级别
            ...
            // 表彰撤销原因
            common.getDictData_tree({id: 'revoke_reason'}).then( res => {
                this.revokeData = res.data.data
            })
        },
        // 树下拉列表
        treechoosenPicker(type, obj) {
            ...
            let field_id = '';
            let field_name = '';
            if(type == 'commend_name') {
                field_id = 'name'
                field_name = 'commendname'
            } else if(type == 'commend_reason') {
                field_id = 'reason'
                field_name = 'commend_reason_name'
            } 
            if(!obj) {
            ...
            if(row.isEdit) {
                this.$refs['form'].validate((valid, vinfo) => {
                    if (valid) {
                        ...
                        datacenter.member_commend_edit(this.formData.g[this.editIndex]).then( res => {
                        ...
                        if(vinfo['g.'+this.editIndex+'.commendname']) {
                            this.$message({ message: vinfo['g.'+this.editIndex+'.commendname'][0].message, type: 'error' });
                        }
                        return false;
                    }
                });
                ...
                // 获取当前编辑新数据
                datacenter.member_commend_info({id: row.id}).then( res => {
                    if(res.data.status  == '200') {
                        this.$set(this.formData.g[index], {}, res.data.data)
                        this.$set(this.formData.g[index], 'isEdit', false)
                        this.set0ValObj(['approval_date', 'revoke_date', 'honorary_title_level', 'approval_org_level', 'revoke_reason'], this.formData.g[index])
        ...
        del(row) {
            if(!row.id) {
                ...
                this.$confirm("确认删除?", "提示", { confirmButtonText: "确定", cancelButtonText: "取消", type: "warning" }).then(() => {
                    datacenter.member_commend_del({id: row.id}).then( res => {
                        if(res.data.status == '200') {
                            this.getData()
                            this.$message({ message: res.data.statusCode, type: 'success' });
                        }else {
        ...
        getData() {
            ...
            this.loading = true
            datacenter['member_commend_list']({id: uid}).then( res => {
                this.loading = false
                this.formData.g = res.data.data
                this.formData.g.map(item => {
                    this.$set(item, 'isEdit', false)
                })
                this.set0Val(['approval_date', 'revoke_date', 'honorary_title_level', 'approval_org_level', 'revoke_reason'])
            }) 
        },
        // 清空为0的数据 全局formData
        set0Val(fieldArr) {
