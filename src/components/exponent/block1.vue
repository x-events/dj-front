            ...
            <div class="_bd flex-1 chartBox">
                <span class="chartHand flex-col">
                    <div class="flex-1 flex-row" v-for="(e,i) in data.result" :key="i" style="padding-top:10px;" @click="goDetail(e)">
                        ...
                        <div class="flex-1">
                            <el-progress :percentage="Number(e.score_percent)" :color="colors[i]" :show-text="false" :stroke-width="10" class="per1" :class="{'noAvg':e.avg_percent === undefined}"></el-progress>
                            <el-progress :percentage="Number(e.avg_percent)" color="#3f79ff" :show-text="false" :stroke-width="10" v-if="e.avg_percent != undefined" class="per2" title="平均线"></el-progress>
                        </div>
...
export default {
    data() {
        return {
            animate:false,
            colors:["#ED3B6A","#7BAE2C","#76E5D9","#B75CDE","#2EB2D7","#ff5252","#c05dd5"],
            data:{
                user:{
                    one:"",
                    total:"",
