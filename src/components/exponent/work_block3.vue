<template>
    <div class="block_wrap_opc withTitle flex-col vbHidden" :class="{'animated fadeInUp faster delay4':animate == 1,'animated fadeOutDown faster':animate == 2}">
        <div class="channelBar red">
            <div class="_title"><i class="wico wi26"></i><strong>社区组织力指数</strong></div>
        ...
        <div class="flex-1 flex-col" style="margin:10px;" v-if="chart == false" v-loading="loading">
            <div class="rank_list flex-1 scrollbar">
                <el-table :data="data.result" style="width: 100%" class="comTable wcs2">
        ...
        <div class="flex-1 pr flex-col" style="margin:10px;" v-if="chart == true">
            <barline :data="chartData" axisPointer="line" :colors="['#e3ed1f']" :areaColors="['#e3ed1f']" :areaColors_status="true" :legendShow="true" />
        </div>
    </div>
</template>
...
<style>
.wcs2 thead { color:#ffffff;}
.wcs2 th:first-child > .cell,.wcs2 th:nth-child(3) > .cell { padding:0;}
.wcs2 th >.cell { font-weight:normal;}
.wcs2 .el-table__header { background-color:transparent;}
.wcs2.el-table--enable-row-hover .el-table__body tr:hover>td { background-color:#466a8e;}
.wcs2.el-table td,.wcs2.el-table th.is-leaf { border-bottom-color:#3b5694;}
</style>