<template>
    <div class="block_wrap_opc withTitle flex-col vbHidden" :class="{'animated fadeInUp faster delay6':animate == 1,'animated fadeOutDown faster':animate == 2}">
        <div class="channelBar orange">
            <div class="_title"><i class="wico wi28"></i><strong>非公党组织组织力指数</strong></div>
            <div class="_ext">平均得分：<span class="bar_score">{{data.avg}}</span> <i class="wico wi29" @click="switchData"></i></div>
        </div>
        <div class="flex-1 flex-col" style="margin:10px;" v-if="chart == false" v-loading="loading">
            <div class="rank_list flex-1 scrollbar">
                <el-table :data="data.result" style="width: 100%" class="comTable wcs3">
        ...
        <div class="flex-1 pr flex-col" style="margin:10px;" v-if="chart == true">
            <barline :data="chartData" axisPointer="line" :colors="['#2ad988']" :areaColors="['#2ad988']" :areaColors_status="true" :legendShow="true" />
        </div>
    </div>
</template>
...
<style>
.wcs3 thead { color:#ffffff;}
.wcs3 th:first-child > .cell,.wcs3 th:nth-child(3) > .cell { padding:0;}
.wcs3 th >.cell { font-weight:normal;}
.wcs3 .el-table__header { background-color:transparent;}
.wcs3.el-table--enable-row-hover .el-table__body tr:hover>td { background-color:#466a8e;}
.wcs3.el-table td,.wcs3.el-table th.is-leaf { border-bottom-color:#3b5694;}
</style>