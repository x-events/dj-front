<template>
  <el-tabs v-model="activeName" class="mytabs">
    <el-tab-pane label="正在学习" name="1" class="course-content">
      ...
      <div v-for="item in learnList.data" :key="item.id" class="flex-row align-center course-item">
        ...
        <div class="info flex-1">
          ...
          <div style="overflow:hidden">
            ...
            <p class="_time">总时长：{{item.time}} &emsp;<span>学时：{{item.credit | scoreFloat}}</span></p>
          </div>
        </div>
    ...
    <el-tab-pane label="已完成" name="2" class="course-content">
...
export default {
  ...
  methods: {
    getCourseList() {
      learn.elective(this.params).then(res => {
        this.learnList = res.data
      })
    },
    getEndList() {
      learn.elective(this.endParams).then(res => {
