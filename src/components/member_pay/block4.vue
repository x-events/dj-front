<template>
    <div class="block_wrap_opc withTitle flex-col vbHidden" :class="{'animated fadeInUp faster delay4':animate == 1,'animated fadeOutDown faster':animate == 2}">
        <div class="channelBar orange">
            <div class="_title"><i class="ion">&#xe60f;</i><strong>党费收缴进度排行</strong></div>
        </div>
                ...
                <el-table :data="data" style="width: 100%" class="comTable zhibupaihang">
                    ...
                    <el-table-column label="进度" width="80" align="right">
                        <template slot-scope="scope">
                            <span class="lightBlue">{{scope.row.percent}}%</span>
                        </template>
                    </el-table-column>
                </el-table>
            </div>
...
export default {
    ...
    methods: {
        getData: function(){
            home.payrank(this.queryParam).then(res => {
...
.zhibupaihang.el-table--enable-row-hover .el-table__body tr:hover>td { background-color:#466a8e;}
.zhibupaihang.el-table td,.zhibupaihang.el-table th.is-leaf { border-bottom-color:#3b5694;}
</style>