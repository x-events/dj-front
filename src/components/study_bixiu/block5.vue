<template>
    <div class="block_wrap_opc withTitle flex-col vbHidden" :class="{'animated fadeInUp faster delay2':animate == 1,'animated fadeOutDown faster':animate == 2}">
        <div class="channelBar orange">
            <div class="_title"><i class="ion">&#xe674;</i><strong>必修课人数和学时</strong></div>
        </div>
        <div class="flex-1 chartBox" style="margin:10px;">
            <barline :data="data" axisPointer="line" :colors="['#2795fa','#f96605']" />
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        getData: function(){
            home.gov_learning_line_chart(this.queryParam).then(res => {
