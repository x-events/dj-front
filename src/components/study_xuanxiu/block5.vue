<template>
    <div class="block_wrap_opc withTitle flex-col vbHidden" :class="{'animated fadeInUp faster delay3':animate == 1,'animated fadeOutDown faster':animate == 2}">
        <div class="channelBar yellow">
            <div class="_title"><i class="ion">&#xe654;</i><strong>最热选学课程</strong></div>
        </div>
        <div class="flex-1 pr flex-col t_blue" style="margin:10px;">
            <el-table :data="data" style="width: 100%" height="100%" stripe class="comTable xxkc">
...
export default {
    ...
    methods: {
        getData: function(){
            home.hotCourseRank(this.queryParam).then(res => {
...
<style>
.xxkc thead { color:#ffffff;}
.xxkc th:first-child > .cell,.xxkc th:nth-child(3) > .cell { padding:0;}
.xxkc th >.cell { font-weight:normal;}
.xxkc .el-table__header { background-color:transparent;}
.xxkc.el-table--enable-row-hover .el-table__body tr:hover>td { background-color:#466a8e;}
.xxkc.el-table td,.xxkc.el-table th.is-leaf { border-bottom-color:#3b5694;}
</style>