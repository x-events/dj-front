<template>
    <div class="block_wrap_opc withTitle flex-col vbHidden" :class="{'animated fadeInUp faster delay5':animate == 1,'animated fadeOutDown faster':animate == 2}">
        ...
        <div class="flex-1 pr flex-col t_blue" style="margin:10px;">
            <el-table :data="data" style="width: 100%" height="100%" stripe class="comTable xxph">
                ...
                <el-table-column prop="credit" label="学时" width="100" align="center"></el-table-column>
            </el-table>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        getData: function(){
            home.CourseStudy(this.queryParam).then(res => {
...
<style>
.xxph thead { color:#ffffff;}
.xxph th:first-child > .cell,.xxph th:nth-child(3) > .cell { padding:0;}
.xxph th >.cell { font-weight:normal;}
.xxph .el-table__header { background-color:transparent;}
.xxph.el-table--enable-row-hover .el-table__body tr:hover>td { background-color:#466a8e;}
.xxph.el-table td,.xxph.el-table th.is-leaf { border-bottom-color:#3b5694;}
</style>