<template>
    <div class="block_wrap_opc withTitle flex-col vbHidden" :class="{'animated fadeInRight faster delay2':animate == 1,'animated fadeOutRight faster':animate == 2}">
        <div class="channelBar yellow" @click="$router.push({name:'工作管理'})">
...
export default {
    ...
    methods: {
        getData: function(){
            home.work_record(this.queryParam).then(res => {
...
.list dl { width:25%; height:72px; float:left; text-align:center; border:#0f476d 1px solid; border-left-width:0; margin:-1px -1px 0 0;font-size:14px;}
.list dd { color:#ffeb08; font-size:20px; padding-top:7px; line-height:1.2;}
.list .unit { font-size:14px; display:inline-block; vertical-align:2px;}
</style>